...
| `ausencias_profesor` | centro_id, profile_id, fecha, fecha_fin, tipo, motivo, estado, aprobada_por, motivo_rechazo, **nota_resolucion**, trimestre (NOT NULL), curso_escolar (NOT NULL), tramo (nullable), created_at | Estado: pendiente/aprobada/rechazada; tipo: baja_medica/permiso/asunto_propio/formacion/sindical/otros. `nota_resolucion`: mensaje de resolución que recibe el profesor (lo redacta el copiloto/agente de RRHH desde la evaluación IA, editable; se muestra en su historial y se envía por push, tanto en aprobación como en rechazo). Las instrucciones para el sustituto van en `sustituciones.observaciones`, NO aquí. `trabajo_alumnos` y `justificante_url` NO existen en el esquema real. |
