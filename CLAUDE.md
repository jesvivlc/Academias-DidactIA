...
> - ⚠️ `supabase/migrations/rls_familia_lockdown_fase2.sql` — **fix privacidad RGPD (fase 2)**: `expedientes_orientacion`, `tramites_orientacion` e `incidencias` pasan a staff-only y las familias obtienen sus datos vía RPCs `SECURITY DEFINER` (`familia_tramites_visibles`, `familia_incidencias_hijos`); `feedback_familias` restringido a propio. **Acoplado a cambios de JS** (`oriRenderTramitesFamilia` → RPC; bloque incidencias de `renderHomeFamilia` → RPC): aplicar SQL **y** desplegar JS a la vez. Diseñada, pendiente de revisión + aplicación.
>
