...
js/
  ...
  consulta-normativa.js initConsultaNormativa (Consulta normativa — RAG): tab staff "⚖️ Consulta normativa"; pregunta → EF `kb-ask` (embebe → RPC `match_kb` → Gemini cita) → respuesta + fuentes con enlace. Corpus `kb_chunks`; ingesta `scripts/ingestar_normativa.mjs`. Fase 1 PoC ✅ activa (migración aplicada, EF desplegada, EBEP de prueba ingestado 2026-06-29)
  ...
  salidas.js            initSalidasPanel (Salidas didácticas): lista + detalle 4 tabs (Dashboard/Cocina/Autobús/Administración), vista familia con autorización por hijo, push, Excel 3 hojas, circular IA. Tablas salidas_didacticas/participantes_salida/notificaciones_salida
  mensajes.js           initMensajes (Mensajería familia↔centro sobre un alumno): vista familia (enviar/hilo) + vista staff (lista de hilos + responder), push bidireccional. Tabla mensajes
  agente.js             window.agenteSustituciones — **Agente de sustituciones** (modo asistido): plan de cobertura del día con asignación global (guardia→equidad, sin conflictos, reparto), briefing + propuesta editable por tramo → confirmar asigna todo (UPDATE+guardia+notify+push). Reutiliza _sustCalcularLibres
  plancobertura.js      window.planCoberturaDia — Plan manual de cobertura por tramo (botón "Plan manual" en Sustituciones); expande filas de día completo, sugiere libres/guardia por tramo, asigna uno a uno
  agentes.js            initAgentesPanel — **Panel central de Agentes** (nav "🤖 Agentes", dirección): tarjeta por agente (sustituciones/RRHH/comunicación/orientación) con dato en vivo (tramos sin cubrir/pendientes/alertas) + botón Lanzar (_agtLanzar→window[fn])
  orientacion-export.js Split de orientacion.js: exportación RGPD (PDF) + estadísticas (Chart.js/PDF/Excel) del módulo Orientación
...
| `tipificar-incidencia` | Clasifica incidencia con Gemini 2.5 Flash según normativa CCAA del centro. Recibe `{ descripcion, centro_id }`. **RAG (Fase 2):** embebe la descripción → `match_kb` → inyecta fragmentos LITERALES de la normativa vigente indexada (p.ej. Decreto 193/2025, LOPIVI) al prompt para que **cite el artículo real**; los devuelve en `fuentes[]`. Devuelve `{ gravedad, tipificacion, medidas_propuestas, informe_borrador, normativa_ref, paradigma, protocolo_previ, **fuentes[]**, alerta_urgente? }` |
...
| `notify-incidencia` | Email a las **familias** del alumno cuando se registra una incidencia. Recibe `{ incidencia_id }`; resuelve las familias vía `familia_alumno`→`profiles` |
...
| `kb-ask` | **RAG normativo (Fase 1).** Identidad/centro/ámbito por JWT+`profiles`+`centros.ccaa`. Recibe `{pregunta}` → embebe con `gemini-embedding-001` (768d, `taskType: RETRIEVAL_QUERY`) → RPC `match_kb` (global del ámbito + normativa del centro, `vigente`) → Gemini 2.5 Flash responde **citando**. Devuelve `{answer, sources[]}`. **Modo `retrieve_only:true`** (Fase 2): salta la respuesta de Gemini y devuelve solo `{sources[]}` con el `texto` completo de cada fragmento — reutilizado por el copiloto de RRHH para citar. Ingesta vía `scripts/ingestar_normativa.mjs` (`RETRIEVAL_DOCUMENT`). ✅ desplegada 2026-06-29. |
...
> **Migraciones pendientes de ejecutar:** (ninguna del RAG). `kb_normativa.sql` (RAG Fase 1: pgvector + `centros.ccaa` + `kb_chunks` + RPC `match_kb`) ✅ **aplicada 2026-06-29**; EF `kb-ask` desplegada y corpus global ingestado (1644 fragmentos / 7 normas vigentes; verificado end-to-end). (`ia_cache.sql` y `calificaciones_competenciales.sql` aplicadas el 2026-06-22.)
>
...
> 🧠 **RAG / base de conocimiento normativo — Fases 1+2 ✅ ACTIVAS (2026-06-29):** pgvector + `kb_chunks` + RPC `match_kb` aplicados; EF `kb-ask` desplegada; tab "⚖️ Consulta normativa" funcionando. Embeddings con `gemini-embedding-001` (768d). **Corpus global (1644 fragmentos, 7 normas vigentes):** EBEP (extracto), Ley 15/2010 y 26/2018 (CV), Decreto 233/2004 y 193/2025 Convivencia (CV), LO 8/2021 LOPIVI, LO 3/2018 LOPDGDD. Fuentes BOE vía XML consolidado (`scripts/_fetch_boe.mjs`) + DOGV (PDF, `pdf-parse` v2). **Fase 2:** el RAG cita artículos reales en `tipificar-incidencia` (recupera+inyecta+`fuentes[]`) y en el copiloto de RRHH (`kb-ask` `retrieve_only` → inyecta EBEP + muestra fuentes). **Próximos pasos:** (1) ampliar corpus (EBEP/LOMLOE completos, NOF de cada centro en `docs/normativa/centro/<slug>/`); (2) **Fase 3**: ingesta desde el módulo Documentos + control de vigencia. Plan completo en **@CLAUDE-ROADMAP.md → "🧠 RAG / Base de conocimiento normativo"**.
