...
- `2026-06-21` — fix(agenda): **XSS en `_agShowDay` — `e.hora` sin escapar** (`js/agenda.js`). El campo `e.hora` se interpolaba directamente en el template literal del card de evento sin pasar por `_agEsc()`. Para eventos de tipo sustitución, `e.hora = s.tramo` (texto libre introducido por admin en BD), lo que permitía inyección HTML. Fix: `_agEsc(e.hora)` en la línea de la etiqueta horaria.
- `2026-06-21 01:25` · `2f3da35` — fix(auditoría): curso_escolar faltante en 7 queries horarios_grupo + escape XSS _escCom
- `2026-06-21` — fix(auditoría): **`curso_escolar` faltante en 7 queries de `horarios_grupo` + escape XSS en `_escCom`**. Barrido sistemático de todos los módulos JS: (1) `comunicados.js` `_escCom` — añadido `.replace(/"/g, '&quot;')` para evitar rotura de atributos HTML `value="..."` en modal `agenteComunicacion`; (2) `mejoras.js` `renderMiHorarioHoy` — query + cache key sin `curso_escolar` mostraban clases de cursos anteriores; (3) `asistencia.js` × 3 queries (`_asistPoblarGrupos`, `_asistGruposDelProfesor`, `_asistRenderLista`); (4) `guardias.js` × 2 queries (`loadBolsaGuardias` + `exportarGuardiasExcel`); (5) `comedor.js` `_detectarContextoProfesor`; (6) `incidencias.js` `_incLoadAlumnosProfesor`; (7) `palette.js` × 2 queries (búsqueda profesor+aula en ⌘K). Confirmados con filtro correcto: `admin.js` (4 queries), `chat.js` (7 queries), `rrhh.js`, `plancobertura.js`, `prevision.js`, `alumnos.js`, `tutoria.js`.
- `2026-06-21 01:12` · `5589636` — fix(sustituciones): badge preciso en todos los filtros + cubierta IS NULL
...
- `2026-06-21 00:16` · `831d117` — feat(calificaciones): evaluación competencial LOMLOE persistente con niveles y copia para Alexia
- `2026-06-21 00:02` · `f58d08d` — feat(calificaciones): comentarios competenciales LOMLOE con IA
- `2026-06-20 23:57` · `a029c8d` — fix(tutorias): vista admin — tabs Citas/Disponibilidad + flujo explicado
- `2026-06-20 23:54` · `916e77a` — fix(config): exponer globals como var para que window.sb/window.ctrId funcionen
- `2026-06-20 23:47` · `840ab6e` — fix(tutorias): mostrar mensaje de error real de Supabase para diagnóstico
- `2026-06-20 23:42` · `8f86e2c` — fix(tutorias): añadir initTutorias a patchShowTab — el panel no cargaba nunca
