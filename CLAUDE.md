...
js/
  ...
  calificaciones.js     initCalificaciones (gradebook): vista profesor (entrada notas; al guardar, push "nueva calificación" a familias de notas que cambiaron vía _calNotificarFamilias) / vista dirección (consulta + export CSV/PDF) / vista familia (solo lectura: tabla pivote asignatura×evaluación de sus hijos, selector si >1 hijo, **boletín PDF** `_calBoletinPDF` con jsPDF+autotable reutilizando helpers de informes.js)
